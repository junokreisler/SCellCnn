...
This module supports a SCellCnn analysis from the command line.
Adjusted to single-cell sequencing data by Elina Jansone, 2023.
...
from cellCnn.utils import get_data, save_results, mkdir_p, get_selected_cells, loadSingleCellDataset, get_sc_data
...
def main():
    ...
    # IO-specific
    parser.add_argument('-f', '--sc_data', required=True,
                        help='CSV file specifying the SCseq file names and their corresponding labels')
    parser.add_argument('-m', '--genes', required=True,
                        help='file specifying the names of genes to be used for analysis')
    ...
    # read in the data
    sc_info = np.array(pd.read_csv(args.sc_data, sep=','))
    gene_names = list(pd.read_csv(args.genes, sep=',').columns)
    ...
    if args.quant_normed:
        args.arcsinh = False
    samples, phenotypes = get_sc_data(args.indir, sc_info, gene_names,
    ...
    logger.info(f"Samples used for model training: {[sc_info[i][0] for i in train]}")
    logger.info(f"Samples used for validation: {[sc_info[i][0] for i in val]}")
    ...
    if args.plot or args.export_selected_cells:
        ...
        plot_filters(results, gene_names, os.path.join(plotdir, 'filter_plots'))
        ...
        filter_info = plot_results(results, train_samples, train_phenotypes,
                                   gene_names, os.path.join(plotdir, 'training_plots'),
                                   filter_diff_thres=args.filter_diff_thres,
        ...
        _v = plot_results(results, valid_samples, valid_phenotypes,
                          gene_names, os.path.join(plotdir, 'validation_plots'),
                          filter_diff_thres=args.filter_diff_thres,
                          filter_response_thres=args.filter_response_thres,
                          stat_test=args.stat_test,
                          group_a=args.group_a, group_b=args.group_b,
        ...
        if args.export_selected_cells:
            ...
            sample_names = [name.split('.txt')[0] for name in list(sc_info[:, 0])]
            # for each sample
